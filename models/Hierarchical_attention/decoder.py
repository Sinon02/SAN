...
class SAN_decoder(nn.Layer):
    ...
    def forward(self, cnn_features, labels, images_mask, labels_mask, is_train=True):
        ...
        c2p_alpha_sum = paddle.zeros((batch_size, 1, height, width))
        c2p_alphas = paddle.zeros((batch_size, num_steps, height, width))

        if is_train:
            parent_hiddens = paddle.zeros(
                (batch_size * (num_steps + 1), self.hidden_size)
            )
            parent_hiddens[:batch_size, :] = self.init_hidden(cnn_features, images_mask)
            c2p_hidden = paddle.zeros((batch_size, self.hidden_size))
            word_alpha_sums = paddle.zeros(
                (batch_size * (num_steps + 1), 1, height, width)
            )
            ...
            for i in range(num_steps):
                ...
                for num in range(relation.shape[0]):
                    if labels[num, -(i + 1), 1] == 2:
                        ...
                        relation[num].item() not in self.struct_dict
                        and relation[num].item() != 0
                    ):
                        relation[num] = 114
                ...
                if word.item() == 2:
                    ...
                    for num in range(structs.shape[1] - 1, -1, -1):
                        if structs[0][num] > self.threshold:
                            struct_list.append(
                                (self.struct_dict[num], hidden, word_alpha_sum)
                            )
